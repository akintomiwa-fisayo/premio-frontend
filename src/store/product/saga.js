...
function* getProductByCategorySaga({ category }) {
  try {
    ...
    if (category !== undefined) {
      const filteredProduct = data.filter((product) => product.categories.some((cat) => cat.value == category));
...
function* getProductByBrandSaga({ brand }) {
  try {
    ...
    if (typeof brand !== 'string') {
      const filterResult = data.filter(({ brand }) => brand.some(({ value }) => filterBrand.includes(value)));
      yield put(getProductsSuccess(filterResult));
    } else {
      const filteredProduct = data.filter((product) => product.brand.some((brandItem) => brandItem.value === brand));
      yield put(getProductsSuccess(filteredProduct));
    }
  } catch (err) {
    yield put(getProductsError(err));
...
function* getProductByKeywordSaga({ keyword }) {
  try {
    ...
    const filteredProduct = data.filter((product) => product.title.toLowerCase().match(regexp));
