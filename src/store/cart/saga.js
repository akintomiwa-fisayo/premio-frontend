...
export const calculateAmount = (obj) => Object.values(obj)
...
function* removeItemSaga(payload) {
  try {
    ...
    const index = localCart.cartItems.indexOf(product);
    localCart.cartTotal -= product.quantity;
...
function* increaseQtySaga(payload) {
  try {
    ...
    const selectedItem = localCart.cartItems.find(
      (item) => item.id === product.id,
    );
    if (selectedItem) {
